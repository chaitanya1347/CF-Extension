...
// ******Getting API KEY and SECRET USING CHROME STORAGE API*******

// let apiKey = "a328233745a5d1fe34bfbbdcfd24718ba496e0db";
// let secret = "a6251eeb10f3f859802a292a986e8c906a7be0b2";
let apiKey,secret;

function toStorUserCredentials(){
    let keysToGet = ['apiKeyText', 'secretKey'];
    // console.log("test");
    chrome.storage.local.get(keysToGet, (result)=>{
      apiKey =  result.apiKeyText;
...
function init(){
    ...
    console.log('reachedFirst');
    let hash = SHA512(`${rand}/user.friends?apiKey=${apiKey}&time=${currTimeUnix}#${secret}`);

    let apiSig = rand + hash;
    let friendsURL = baseURL + `user.friends?apiKey=${apiKey}&time=${currTimeUnix}&apiSig=${apiSig}`;
        ...
        for(let i=1; i<=20; i++){
            ...
            if(i%2){
                tableRowElement.innerHTML = `
                    <td class = "left dark">${responseJSON.result[i]}</td>
                    <td class = " dark">testing</td>
                    <td class = " dark">PASS</td>
                `;
            }
            else {
                tableRowElement.innerHTML = `
                <td class = "left ">${responseJSON.result[i]}</td>
                <td class = " ">testing</td>
                <td class = " ">PASS</td>
                `;
            }
...
// toStorUserCredentials().then(init);
toStorUserCredentials();